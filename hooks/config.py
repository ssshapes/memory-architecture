...
# ----------------------------------------------------------- derived index ----
# MEMORY.md is generated by build_memory_index.py, not written by hand: pinned
# types first, then every other memory ranked by measured use (the recall log)
# plus inbound wikilinks, filled to a byte budget. The budget sits under the
# validator's INDEX_BLOCK_BYTES on purpose: the generator must never produce a
# page the write-time gate would refuse. Memories that miss the page are still
# retrieved per prompt; the page is only what is ALWAYS loaded.
INDEX_BUDGET_BYTES = 17_000       # whole page, trailer included
INDEX_TRAILER_RESERVE = 2_600     # bytes kept for the below-the-fold name list
INDEX_HOOK_LEN = 90               # characters of hook per line
INDEX_PIN_TYPES: tuple[str, ...] = ("feedback",)   # frontmatter type: values always on the page
INDEX_NEWBORN_DAYS = 7            # a memory younger than this gets the floor below
INDEX_NEWBORN_FLOOR = 25.0        # score floor, so a newborn spends a week on the page
INDEX_STALE_DAYS = 8              # lint flags a derived page older than this
INDEX_SYNC_GRACE_S = 600          # lint ignores memory files younger than this in the coverage check
